	...
	*/
	...
	void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
	{
		...
		for (unsigned int i = 0; i < contacts->contact_size(); ++i)
		{
			...
			*/
			...
			// rewardHistory = REWARD_LOSS; // task 1
	...
	bool ArmPlugin::updateAgent()
	{
		...
		*/
		int inc_val = (action%2==0) ? 1 : -1;
		float joint = ref[action/2] + actionJointDelta * inc_val ;; // TODO - Set joint position based on whether action is even or odd.
			...
			*/ 
			...
			if(!checkGroundContact)
			{
				...
				if( episodeFrames > 1 )
				{
					...
					// rewardHistory = avgGoalDelta * REWARD_WIN; //task 1
					rewardHistory = avgGoalDelta * 5 - 0.5; // -3.0 reward as more time taken
